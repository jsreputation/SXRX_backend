...
async function main() {
  ...
  const xml = buildEnvelope('GetCharges', requestXml);

  const response = await postSoap({ endpoint, methodName: 'GetCharges', xml });
  console.log(`Status: ${response.status}`);

  const blocks = extractBlocks(response.data, 'ChargeData');
  if (!blocks.length) {
    console.log(response.data);
    return;
  }
  const items = blocks.map(extractFields);
  console.log('Charges:', JSON.stringify(items, null, 2));
}
