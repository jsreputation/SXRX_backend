...
async function main() {
  ...
  const xml = buildEnvelope('GetTransactions', requestXml);

  const response = await postSoap({ endpoint, methodName: 'GetTransactions', xml });
  console.log(`Status: ${response.status}`);

  const blocks = extractBlocks(response.data, 'TransactionData');
  if (!blocks.length) {
    console.log(response.data);
    return;
  }
  const items = blocks.map(extractFields);
  console.log('Transactions:', JSON.stringify(items, null, 2));
}
